...
// Simplificado para evitar deslogamentos indesejados
  ...
  try {
    ...
    if (activeSessionCheck.rows.length === 0) {
      ...
      if (anyActiveSession.rows.length > 0) {
        ...
        console.log(`Sessão ${sessionId} inválida para usuário ${userId} - existe outra sessão ativa`);
        ...
        // Não existe nenhuma sessão ativa, registrar esta como ativa
        try {
          await pool.query(
            'INSERT INTO active_sessions (user_id, session_id, user_email, last_activity) VALUES ($1, $2, $3, CURRENT_TIMESTAMP)',
            [userId, sessionId, userEmail]
          );
          console.log(`Sessão ${sessionId} registrada como ativa para usuário ${userId}`);
        } catch (error) {
          console.error('Erro ao registrar sessão ativa:', error);
        }
      }
    } else {
      // Esta sessão está ativa, atualizar timestamp
      try {
        await pool.query(
          'UPDATE active_sessions SET last_activity = CURRENT_TIMESTAMP WHERE session_id = $1',
          [sessionId]
    ...
    // Sempre permitir o acesso se chegou até aqui
    next();
  } catch (error) {
